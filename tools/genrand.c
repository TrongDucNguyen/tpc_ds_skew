...
{
	return ((double)next_random(stream) / (double)MAXINT);
}
...
genrand_integer(int *dest, int dist, int min, int max, int mean, int stream)
{
	...
	case DIST_UNIFORM:
		...
		res += min;*/
		for (i = 0; i < 2; i++)
			fres += (double)(next_random(stream) % (max - min + 1)) / 2;
		res = min + (int)fres;
		break;
	...
	decimal_t * mean, int stream)
{
	...
	case DIST_UNIFORM:
		...
		for (i = 0; i < 2; i++)
			fres += (double)(next_random(stream) % (max->number - min->number + 1)) / 2;
		res.number = min->number + (int)fres;
